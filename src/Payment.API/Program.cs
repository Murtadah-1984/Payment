...
if (args.Contains("--generate-report"))
{
...
Log.Logger = new LoggerConfiguration()
    ...
    .Enrich.WithThreadId()
    .Enrich.With(new Extensions.TraceIdentifierEnricher()) // Adds TraceId and SpanId from Activity.Current
...
healthChecksBuilder.AddCheck<HealthChecks.PaymentProviderHealthCheck>(
    "payment-providers",
    tags: new[] { "provider", "live" });

// Add disk space health check
healthChecksBuilder.AddCheck<HealthChecks.DiskSpaceHealthCheck>(
    "disk-space",
...
app.MapGraphQL()
    ...
    {
        Tool = builder.Environment.IsDevelopment()
            ? new GraphQLToolOptions { Enable = true }
            : new GraphQLToolOptions { Enable = false }
    });
